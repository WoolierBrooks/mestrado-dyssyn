...
def baseline_features(scores):
    ...
    return np.array([
        ...
        np.std(scores),
        q[2],               # mediana
        q[4] - q[0],        # IQR estendido
        wasserstein_distance(scores, np.linspace(0,1,len(scores)))
    ])
...
def run_experiment():
    ...
    pd.DataFrame(rows).to_csv("baseline_02.csv", index=False)
    print("\n✅ Finalizado → baseline_02.csv")
