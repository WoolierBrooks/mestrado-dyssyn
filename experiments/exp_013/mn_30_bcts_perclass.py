...
# ============================================================
def conditional_class_features(scores_matrix, y_true, n_classes):
    """
    Extrai estatísticas dos scores condicionadas à classe verdadeira.

    Para cada classe verdadeira c:
        usa scores[y == c, :]
        e extrai stats apenas do score da classe c

    Output shape: (4 * n_classes * n_classes,)
    """
    feats = []
    ...
    for ds_name in datasets:
        ...
        if n_classes_real not in regressores_treinados:
            moss_path = os.path.join(
                MOSS_DIR,
                f"moss_m_mn_{n_classes_real}.pkl"
            )

            if not os.path.exists(moss_path):
                print(f"⚠️ moss_m_lite_{n_classes_real}.pkl não encontrado.")
                ...
                for (alpha_prev, _), curves in synthetic_distributions.items():
                    for scores_matrix in curves:
                        # rótulos verdadeiros artificiais
                        y_synth = np.repeat(
        ...
        for idx_batch in tqdm(
            ...
            p_real = np.array(list(p_real.values()))

            scores_batch = clf.predict_proba(Xte[idx_batch])

            # ---------- MoSS ----------
            if moss_model is not None:
                f_vec = conditional_class_features(
                    scores_batch,
                    yte[idx_batch],
                    n_classes_real
                ).reshape(1, -1)

                p_pred = moss_model.predict(f_vec)[0]
