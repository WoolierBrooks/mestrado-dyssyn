...
def run_experiment():
    ...
    if os.path.exists(OUTPUT_CSV):
        ...
        # NOVO: pares (dataset, repetição) já presentes no CSV — usado
        # dentro do loop, sem precisar reabrir/checar o arquivo de novo
        reps_completed = set(zip(df_prev["dataset"], df_prev["repetition"]))

        print(f"\n♻️  Checkpoint encontrado: {len(datasets_done)} dataset(s) completo(s) "
            f"({N_REPETITIONS} reps), {len(rows)} linhas carregadas de {OUTPUT_CSV}")
    ...
    for ds_name, source, meta in all_entries:
        ...
        for rep in range(N_REPETITIONS):
            ...
            # ANTES (quebra):
            if os.path.exists(OUTPUT_CSV):
                ja_feito = ((df_prev["dataset"] == ds_name) &
                            (df_prev["repetition"] == rep + 1)).any()
                if ja_feito:
            ...
            reps_completed.add((ds_name, rep + 1))  # NOVO
