...
def run_experiment():
        ...
        for ds in datasets:
            ...
            for idx, _ in app.split(Xte, yte):
                ...
                prev_pred = reg.predict(
                    pymfe_features(scores).reshape(1, -1)
                )[0]
    ...
    pd.DataFrame(rows).to_csv("comparacao_mfe_only.csv", index=False)
    print("\n✅ Finalizado → comparacao_mfe_only.csv")
